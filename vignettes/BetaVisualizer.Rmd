...
fuel efficiency, measured in miles per gallon (MPG), we will apply our data
...
df_mtcars[c("cyl","vs","am","gear")] =
  lapply(df_mtcars[c("cyl","vs","am","gear")] , factor)                         # convert into factor variables
...
(specifically, the straight engine) and vehicle weight influence fuel efficiency
the most, with effects of these two variables remaining consistent when 
...
analytical lens, displacement and horsepower emerge as the third and second most
influential factors, respectively. This shift in variable significance can be
attributed to the fact that the differences in displacement and horsepower among 
the majority of vehicles do not typically equate to  a mere 1 cubic inch or 
1 horsepower. Consequently, this phenomenon underscores the criticality of 
considering the distribution of variables in the interpretation of regression 
outcomes, as reliance on per-unit interpretations may lead to misconceptions.
...
### 1.4 Customizing pallete and title, modifying default effect size calculations
...
```
### 2.2 Default side-by-Side plot, CIs and 99% confidence interval, 
...
right plot). Concurrently, the variable 'male gender' which predominates 
...
We will use LASSO regression to understand how car characteristics influence 
sales price using a data set from 93 Cars on Sale in the USA in 1993. 
...
levels(df_glmnet$Origin)                                                        # check level attributes of a factor variable
...
x_lasso = x_lasso[, -1]                                                         # remove intercept as glmnet adds it

ndim_lasso=dim(x_lasso)[1]

cv_model_lasso = cv.glmnet(x_lasso, y_lasso, family="gaussian", alpha=1)        # alpha=1 specifies LASSO regression
...
### 3.3 Check correlations for numeric variables of interest

```{r,fig.height=9,fig.width=12, warning=F}
...
corPlot(df_glmnet_num,xlas=2, cex=1.1)
...
### 3.4 Default LASSO plots with effect size calculated for Q2-min difference

```{r, fig.height=6,fig.width=12, warning=F}

grid.arrange(vis_reg(best_model_lasso,eff_size_diff=c(1,3), 
                     glmnet_fct_var="Originnon-USA")$"SidebySide")              # note the naming pattern for categorical variables
    

```

Note that the 'Weight' variable is retained and remains consistent ($0$) 
...
We will employ the Stanford Heart Transplant data to demonstrate the 
construction of CIs for objects of the glmnet class.
...
*How does the Odds Ratio (OR) change with a __unit__ increment in the variables 
under scrutiny?

*How does the OR vary in response to alterations exceeding a single 
unit, such as the disparity between the first (Q1) and third (Q3) quartiles
...
the __'round_func'__ parameter allows for the specification of rounding the 
...
__detransform__ that carries out the re-scaling and de-centering process for 
effective size difference calculations. Alternatively, consider rounding down
or up before passing the data to the function. 

### 3.6.5 A note on data scaling and centering ('glmnet')
...
values and do not encompass the estimated coefficients. 
...
```
### 3.6.8 A second note on data scaling and centering ('fixedLassoInf') 
...
which is 7.217 - 6.519 = 0.698 ( __summary(heart_df_filtered$sqrt.age)__ ), 
